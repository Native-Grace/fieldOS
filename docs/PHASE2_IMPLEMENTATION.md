...
| VoiceProcessing queue contract | **Deployed / live-verified** — Queue calls `VoiceProcessing.executePipeline(jobRow)`. Transcription uses **OpenAI Whisper** via `OpenAI.transcribeAudio` (`OPENAI_API_KEY`). No Gemini on this path. Drive resolve prefers `recording_drive_file_id`; transcript/transcription writeback is header-safe. Status `Invalid` recordings are skipped; `Processed` + transcript are idempotent skips. Success sets `processing_status=Completed` and clears `processing_error`. Failures rethrow to Queue with sanitised `recording_id` context. Structured GPT summary / job-line writeback remains **out of scope**. **Live verification (job `21759f5d`):** end-to-end Completed; invalid stubs skipped; valid recording transcribed. |
