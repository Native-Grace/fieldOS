...
| VoiceProcessing queue contract | **Repo fix ready (OpenAI)** — `VoiceProcessing.executePipeline(jobRow)` facade + FieldOS recording bridge. Transcription uses **OpenAI Whisper** via `OpenAI.transcribeAudio` (`OPENAI_API_KEY`). Gemini is not used on this path. Structured GPT summary / job-line writeback remains **out of scope**. Requires Apps Script Save before any retry. **Do not retry until OpenAI path is deployed.** |
