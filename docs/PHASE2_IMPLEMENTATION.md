...
| VoiceProcessing queue contract | **Deployed / live-verified** — Whisper path live. **Phase 3A (repo):** GPT structured summary via `OpenAI.chatComplete` writes `ai_transcript` / `ai_summary` / list fields / `ai_confidence_score`, then `Completed`. Invalid recordings skipped. Requires Apps Script Save of `VoiceProcessing.js` (+ diagnostics dry-run optional). Do not auto-reprocess production jobs. |
