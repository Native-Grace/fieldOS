...
const DB = {
   ...
   * @param {{alreadyLocked?: boolean}=} options Pass alreadyLocked when caller holds Utils.withLock
   *   (ScriptLock is not re-entrant — nested insert would deadlock).
   */
  insertRecord: function(tableName, recordObj, options) {
    const runInsert = () => {
    ...
    if (options && options.alreadyLocked) {
      return runInsert();
    }
    return Utils.withLock(`INSERT_${tableName}`, 10000, runInsert);
  },
